...
/**
 * Опрделение показателей эффективности двигателей
 */
class Determination(
    ...
    val specificGravityOnStartFromEarth by lazy {
        with(projectParams) {
            specificGravityInVoidFirst -
                    ((fuel.gasConstant * fuel.burningPoint / specificGravityCalculatedFirst / Constants.FREE_FALL_ACCELERATION.pow(2.0)) *
                            (engineNozzleShearPressure.first / pressureInTheCombustionChambersOfEngines.first).pow((fuel.adiabaticValue - 1.0) / fuel.adiabaticValue) *
